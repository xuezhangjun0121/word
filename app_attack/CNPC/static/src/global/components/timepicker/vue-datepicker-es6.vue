//reference: https://github.com/hilongjw/vue-datepicker
<style scoped>
.datepicker-overlay {
  position: fixed;
  width: 100%;
...
<template>
  <div class="cov-vue-date">
    <div class="datepickbox">
      <input type="text" class="form-control cov-datepicker" readonly="readonly" :placeholder="datePlaceholder" v-model="date.time" :required="required" @click="showCheck" @foucus="showCheck" :style="option.inputStyle ? option.inputStyle : {}" />
    </div>
    <div class="datepicker-overlay" v-if="showInfo.check" @click="dismiss($event)" v-bind:style="{'background' : option.overlayOpacity? 'rgba(0,0,0,'+option.overlayOpacity+')' : 'rgba(0,0,0,0.5)'}">
      <div class="cov-date-body" :style="{'background-color': option.color ? option.color.header : '#3f51b5'}">
        ...
        <div class="button-box">
          ...
          <span @click="clearDate">{{option.buttons? option.buttons.clear : 'clear' }}</span>
...
function hours (date) {
  let list = []
  let hour = 24
  let curHour = date?moment(date).get('hour'):moment().get('hour')
  while (hour > 0) {
    hour--
    if(hour == curHour){
      list.push({
      ...
      hourChecked = hour < 10 ? '0' + hour : hour
    }else {
      list.push({
        checked: false,
        value: hour < 10 ? '0' + hour : hour
...
function mins (date) {
  let list = []
  let min = 60
  let curMinute = date?moment(date).get('minute'):moment().get('minute')
  while (min > 0) {
    min--
    if(min == curMinute){
      list.push({
      ...
      minuteChecked = min < 10 ? '0' + min : min
    }else {
      list.push({
        checked: false,
        value: min < 10 ? '0' + min : min
...
export default {
  ...
  data () {
    return {
      hours: hours(this.date.time),
      mins: mins(this.date.time),
      showInfo: {
        hour: false,
        day: false,
        month: false,
  ...
  methods: {
    ...
    showDay (time) {
      ...
      days = this.limitMinMax(this.minDate, this.maxDate, days)
    ...
    checkBySelectDays (d, days) {
      if(!this.date.time){
        days.forEach(day => {
          if (this.checked.year === moment().format('YYYY') && this.checked.month === moment().format('MM') && d === Math.ceil(moment().format('D'))) {
            days[d - 1].checked = true
          }
        })
      }else{
        days.forEach(day => {
          if (this.checked.year === moment(this.date.time).format('YYYY') && this.checked.month === moment(this.date.time).format('MM') && d === Math.ceil(moment(this.date.time).format('D'))) {
            days[d - 1].checked = true
          }
        })
      }
    ...
    limitMinMax (min, max, days) {
      if (min || max) {
        days.map((day) => {
          if (this.getLimitCondition(min, max, day)) {
            day.unavailable = true
          }
        })
      }
    ...
    getLimitCondition (min, max, day) {
      let tmpMoment;
      if(day.action){
          day.action == "previous"?tmpMoment = moment(this.checked.year + '-' + this.pad(parseInt(this.checked.month)-1) + '-' + this.pad(day.value)):tmpMoment = moment(this.checked.year + '-' + this.pad(parseInt(this.checked.month)+1) + '-' + this.pad(day.value))
      } else {
        tmpMoment = moment(this.checked.year + '-' + this.pad(this.checked.month) + '-' + this.pad(day.value))
      }         
      
      if (min && !max) {
        return !tmpMoment.isAfter(min)
      } else if (!min && max) {
        return !tmpMoment.isBefore(max)
      } else {
        return !tmpMoment.isBetween(min, max)
      }
    },
    ...
    // },
    checkDay (obj) {
      this.$emit('oldtime', this.date.time)
    ...
    picked () {
      if (this.option.type === 'day' || this.option.type === 'min') {
        ...
        if (moment(ctime) > moment(this.maxDate)) {
          ctime = moment(this.maxDate).format('YYYY-MM-DD HH:mm')
          this.hours = hours(this.date.time)
          this.mins = mins(this.date.time)
        }
        if (moment(ctime) < moment(this.minDate).add(1, 'day')) {
          ctime = moment(this.minDate).add(1, 'day').format('YYYY-MM-DD HH:mm')
          this.hours = hours(this.date.time)
          this.mins = mins(this.date.time)
        }
